...
class SolersDock : public PanelContainer {
	...
	PopupMenu *message_menu = nullptr;
	int64_t message_menu_event_id = -1;
	...
	Control *_append_user_message(const String &p_message, const Array &p_attachments = Array(), int64_t p_event_id = -1);
	void _on_user_message_gui_input(const Ref<InputEvent> &p_event, int64_t p_event_id);
	void _on_user_message_rewind(const String &p_text, int64_t p_event_id);
	void _on_message_menu_id_pressed(int p_id);
	void _reload_branched_session(const Dictionary &p_data);
